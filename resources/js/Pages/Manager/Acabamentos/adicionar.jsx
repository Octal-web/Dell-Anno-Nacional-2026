...
const Page = () => {
    ...
    const inputItems = [
        ...
        [{ titulo: 'Imagem', name: 'img', tamanho: 'col-span-12 md:col-span-6', tipo: 'imagem', crop: true, largura: 800, altura: 432 }],
    ];

    useEffect(() => {
        const initialData = {};
        inputItems.forEach(group => group.forEach(item => initialData[item.name] = item.tipo === 'check' ? false : ''));
        setData(initialData);
    }, []);

    const handleSubmit = (e) => {
        e.preventDefault();
        post(route('Manager.Acabamentos.novo'), { preserveScroll: true });
    };
