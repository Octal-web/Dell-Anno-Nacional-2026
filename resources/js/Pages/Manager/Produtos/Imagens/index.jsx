...
const Page = () => {
    ...
    const [state, setState] = useState(produto.imagens);
    ...
    useEffect(() => {
        if (isReadyToUpload && data.images.length > 0) {
            post(route('Manager.Produtos.Imagens.novo', { id: produto.id }), {
                preserveScroll: true,
                preserveState: false,
            });
    ...
    useEffect(() => {
        if (isUpdated) {
            post(route('Manager.Produtos.Imagens.ordenar', {id: produto.id}), {
                preserveScroll: true,
            });
            setIsUpdated(false);
        }
                        ...
                        >
                            {state.map((image, index) => (
                                <div key={index} className="relative h-full pb-12">
                                    <IndividualImage key={index} individualContent={image} controller="Produtos.Imagens" crop={false} />
                                </div>
                            ))}
                        </ReactSortable>
                    </div>
