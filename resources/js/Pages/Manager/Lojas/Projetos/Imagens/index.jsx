...
const Page = () => {
    ...
    useEffect(() => {
        if (isReadyToUpload && data.images.length > 0) {
            post(route('Manager.Lojas.Projetos.Imagens.novo', { id: projeto.id }), {
                preserveScroll: true,
                preserveState: false,
            });
    ...
    useEffect(() => {
        if (isUpdated) {
            post(route('Manager.Lojas.Projetos.Imagens.ordenar', {id: projeto.id}), {
                preserveScroll: true,
            });
            setIsUpdated(false);
        }
    ...
    const breadcrumbItems = [
        ...
        { label: projeto.nome, link: 'Manager.Lojas.Projetos.editar', params: { id: projeto.id } }
    ];
            ...
            <div className="rounded-sm border border-stroke bg-white px-5 py-5 shadow-md">
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                    ...
                    <div className="lg:col-span-1 min-h-[calc(100vh-13rem)]">
                        <ImageUploader onUpload={handleImageUpload} crop={false} size={{ largura: 450, altura: 357 }} />
                    </div>
                </div>
            </div>
        </AdminLayout>
