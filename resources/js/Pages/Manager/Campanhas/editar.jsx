...
import { faBullhorn, faSave, faArrowLeft } from '@fortawesome/free-solid-svg-icons';
...
const Page = () => {
    const { idioma, idiomas, campanha } = usePage().props;

    const { data, setData, post, processing, errors } = useForm(campanha);

    const breadcrumbItems = [
        { label: 'Home', link: 'Manager.Home.index' },
        { label: 'Campanhas', link: 'Manager.Home.index' },
    ];

    const inputItems = [
        [{ titulo: 'Título', name: 'titulo', tamanho: 'col-span-12 lg:col-span-8', tipo: 'texto', max: 120 }],
        [{ titulo: 'Link', name: 'link', tamanho: 'col-span-12 lg:col-span-8', tipo: 'link', max: 120 }],
        [{ titulo: 'Descrição', name: 'descricao', tamanho: 'col-span-12 lg:col-span-8', tipo: 'texto_longo', editor: true, 'toolbar': ['Bold', 'Italic'], max: 250 }],
        [{ titulo: 'Imagem ', name: 'img', tamanho: 'col-span-12 md:col-span-6 lg:col-span-4', tipo: 'imagem', crop: true, largura: 1100, altura: 730, imagem: campanha.imagem }]
    ];
    ...
    const handleSubmit = (e) => {
        ...
        post(route('Manager.Campanhas.atualizar', {id: campanha.id, lang: idioma_url}), {
            preserveScroll: true,
        });
        console.log(data);
    ...
    return (
        <AdminLayout>
            <Breadcrumb icon={faBullhorn} items={breadcrumbItems} current="Editar" idioma={idioma.codigo} idiomas={idiomas} id={campanha.id} />
