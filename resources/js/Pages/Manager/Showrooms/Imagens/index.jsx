...
const Page = () => {
    const { showroom } = usePage().props;

    const [state, setState] = useState(showroom.imagens);
    ...
    useEffect(() => {
        if (isReadyToUpload && data.images.length > 0) {
            post(route('Manager.Showrooms.Imagens.novo', { id: showroom.id }), {
                preserveScroll: true,
                preserveState: false,
            });
    ...
    useEffect(() => {
        if (isUpdated) {
            post(route('Manager.Showrooms.Imagens.ordenar', {id: showroom.id}), {
                preserveScroll: true,
            });
            setIsUpdated(false);
        }
    ...
    const breadcrumbItems = [
        ...
        { label: showroom.nome, link: 'Manager.Showrooms.editar', params: { id: showroom.id } }
    ];
            ...
            <div className="rounded-sm border border-stroke bg-white px-5 py-5 shadow-md">
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                        ...
                        >
                            {state.map((image, index) => (
                                <div key={index} className="relative h-full pb-12">
                                    <IndividualImage key={index} individualContent={image} controller="Showrooms.Imagens" crop={false} />
                                </div>
                    ...
                    <div className="lg:col-span-1 min-h-[calc(100vh-13rem)]">
                        <ImageUploader onUpload={handleImageUpload} crop={false} size={{ largura: 800, altura: 510 }} />
                    </div>
                </div>
            </div>
        </AdminLayout>
