        ...
        <>
            <section className="pt-8 md:pt-12 2xl:pt-20">
                <div className="container max-w-x-large">
                    <h1 className="text-4xl md:text-5xl 2xl:text-[55px] font-light uppercase sm:tracking-wide mb-6 md:mb-10">
                        {content.titulo}
                    </h1>
                    <p className="font-secondary font-light max-sm:text-justify sm:tracking-wide sm:leading-loose whitespace-pre-line max-w-[830px] mb-8 2xl:mb-10">
                        {content.texto}
                    </p>
                </div>
            ...
            <section className="pt-10 md:pt-20 pb-20 md:pb-30 2xl:pb-40">
                <div className="container max-w-x-large">
                    {stores.length ? (
                        <div
                            className={`grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-16 md:gap-y-20${loading ? " opacity-50" : ""}`}
                        >
                            {stores.map((store, index) => {
                                const isExternalLink = store.link_lp;
                                const linkUrl = isExternalLink
                                    ? store.link_lp
                                    : route("Lojas.loja", { slug: store.slug });
                                ...
                                const titleElement = (
                                    <h3 className="text-2xl font-light max-sm:tracking-tight uppercase mx-2 sm:mx-4">
                                        Dell Anno{" "}
                                        {store.estado
                                            ? store.cidade +
                                              " - " +
