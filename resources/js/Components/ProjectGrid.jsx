...
export const ProjectGrid = ({ project }) => {
    const [lightboxOpen, setLightboxOpen] = useState(false);
    const [lightboxIndex, setLightboxIndex] = useState(0);

    const lightboxSlides = project.imagens.map((imagem) => ({
        src: imagem.imagem
    }));
    ...
    return (
        <>
                        ...
                        {project.imagens.map((imagem, index) => (
                            <Reveal scale={true} key={index} delay={index * 0.8} className="relative overflow-hidden">
                                ...
                                >
                                    ...
                                    <div className="hidden absolute inset-0 md:flex items-center justify-center opacity-0 transition-all duration-300 group-hover:opacity-100 z-10">
            ...
            <p className="font-secondary text-justify md:text-center font-light sm:leading-loose whitespace-pre-line max-w-5xl mx-auto pb-20 max-w-[90vw]">{project.detalhes}</p>
        </>
    );
};