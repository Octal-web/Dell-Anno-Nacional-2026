...
export const ProjectGallery = ({ text, slides }) => {
    const [lightboxOpen, setLightboxOpen] = useState(false);
    const [currentIndex, setCurrentIndex] = useState(0);

    const lightboxSlides = slides.map(slide => ({
        src: slide.imagem_zoom
    }));
    ...
    return (
        <section className="pb-36">
            <div className="container max-w-x-large">
                <p className="font-secondary text-center font-light leading-loose whitespace-pre-line max-w-5xl mx-auto mb-30">{text}</p>
                {slides.length ? (
                        ...
                        >
                            {slides.map((slide, index) => 
                                <SwiperSlide key={index}>
                                    <div 
                                        className="group aspect-[97/110] overflow-hidden cursor-pointer relative"
